		 ...
		 if(data.length != 0){
			...
			if(emp!=null){
				let dict =     {
					...
					"picture": emp.picture,
					"reqId": e._id,
					"date":  dateLib.format(e.createdAt,'MM-DD-YYYY')
				}
				userArray.push(dict);
			}
...
router.post('/orgRequestAction', async(req,res) => {
	...
	try{
	   ...
	   if(req.body.status == '1') {
		...
		let emailContent = "Congratulations! "+ orgData.organizationName + " Organization has accepted your joing request, you are now a member in this organization.";
		...
		if(errors.indexOf(empDetails.deviceToken) == -1){
		   sendFCM(empDetails.deviceToken, 'Joining request accepted!', 'Congratulations! '+ orgData.organizationName + ' Organization has accepted your joing request, you are now a member in this organization.');
		}
